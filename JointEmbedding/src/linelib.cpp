...
void line_node::init(char *file_name, int vector_dim)
{
	...
	for (a = 0; a < node_size; a++) {
		...
		for (b = 0; b < vector_size; b++) {
			...
			norm += _vec[a * vector_size + b] * _vec[a * vector_size + b];
		}
		for (b = 0; b < vector_size; b++)
			_vec[a * vector_size + b] /= sqrt(norm);
	}
...
real line_trainer::train_sample(real alpha, real margin, double(*func_rand_num)(), unsigned long long &rand_index)
{
	int target, u, v, index;
	real f, g, h, step, obj = 0;
	line_node *node_u = phin->node_u, *node_v = phin->node_v;

	u = smp_u.draw(func_rand_num(), func_rand_num()); // positive center node u
	if (u_nb_cnt[u] == 0) return 0;
	index = (int)(smp_u_nb[u].draw(func_rand_num(), func_rand_num()));
	v = u_nb_id[u][index]; // positive context node v
	...
	for (int d = 0; d < neg_samples; d++)
	{
		...
		target = neg_table[(rand_index >> 16) % neg_table_size]; // negative context node v'
		if (target == v) continue;
		
		f = node_u->vec.row(u) * node_v->vec.row(v).transpose(); // f = cos(u, v)
		h = node_u->vec.row(u) * node_v->vec.row(target).transpose(); // h = cos(u, v')

		if (f - h < margin) {
			obj += margin - (f - h);

			// compute context node gradient
			neu1e = node_v->vec.row(v) - f*node_u->vec.row(u) + h*node_u->vec.row(u) - node_v->vec.row(target);

			// update positive context node
			grad = node_u->vec.row(u) - f*node_v->vec.row(v); // negative Riemannian gradient
			step = 1 - f; // cosine distance
			node_v->vec.row(v) += alpha * step * grad;

			g = node_v->vec.row(v) * node_v->vec.row(v).transpose();
			g = sqrt(g);
			node_v->vec.row(v) /= g;

			// update negative context node
			grad = h*node_v->vec.row(target) - node_u->vec.row(u); // negative Riemannian gradient
			step = 2 * h; // 2 * cosine similarity
			node_v->vec.row(target) += alpha * step * grad;

			g = node_v->vec.row(target) * node_v->vec.row(target).transpose();
			g = sqrt(g);
			node_v->vec.row(target) /= g;

			// update positive center node
			step = 1 - (f - h);
			node_u->vec.row(u) += alpha * step * neu1e;

			g = node_u->vec.row(u) * node_u->vec.row(u).transpose();
			g = sqrt(g);
			node_u->vec.row(u) /= g;
		}
	}
