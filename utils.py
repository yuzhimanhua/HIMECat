...
def load_word_embedding(sentence_matrix, vocabulary_inv, dataset_name, embedding_name, suffix='', mode='skipgram',
	...
	for word in embedding_model.vocab:
		if word.startswith('$LABL_'):
			class_name = word[6:]
			class_emb[class_name] = embedding_model[word] / np.linalg.norm(embedding_model[word])

	return embedding_weights, class_emb


def load_embedding(x, vocabulary_inv, dataset_name, embedding_name, node, suffix='', mode='skipgram',
						 num_features=100, min_word_count=5, context=5):
	print(f'Training embedding for node {node.name}')

	embedding_weights, class_emb = load_word_embedding(x, vocabulary_inv, dataset_name, embedding_name, suffix, mode,
...
def proceed_level(x, sequences, wstc, args, pretrain_epochs, self_lr, decay, update_interval,
				...
				len_std, num_doc, interp_weight, vocabulary_inv, common_words, kappa, word_embedding_dim):
	...
	for parent in parents:
		...
		if class_tree.embedding is None:
			load_embedding(x, vocabulary_inv, dataset_name=args.dataset, embedding_name=args.embedding, node=class_tree)
		parent.embedding = class_tree.embedding
		parent.class_emb = class_tree.class_emb

		wstc.instantiate(class_tree=parent, word_embedding_dim=word_embedding_dim)
		...
		if parent.model is not None:
			...
			seed_docs, seed_label = bow_pseudodocs(parent.children, expand_num, background_array, doc_length, len_avg,
													len_std, num_doc, interp_weight, vocabulary_inv, parent.embedding, parent.class_emb, kappa, save_dir)
